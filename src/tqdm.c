...
void tqdm_load_env_vars(tqdm_params_t *params) {
    ...
    if ((env_val = getenv("TQDM_ASCII")) != NULL) {
        params->ascii = (strcasecmp(env_val, "true") == 0 || strcmp(env_val, "1") == 0);
    }
    if ((env_val = getenv("TQDM_DISABLE")) != NULL) {
        params->disable = (strcasecmp(env_val, "true") == 0 || strcmp(env_val, "1") == 0);
    }
    if ((env_val = getenv("TQDM_UNIT")) != NULL) {
        if (params->unit) free(params->unit);
        params->unit = strdup(env_val);
    }
    if ((env_val = getenv("TQDM_UNIT_SCALE")) != NULL) {
        params->unit_scale = (strcasecmp(env_val, "true") == 0 || strcmp(env_val, "1") == 0);
    }
    if ((env_val = getenv("TQDM_DYNAMIC_NCOLS")) != NULL) {
        params->dynamic_ncols = (strcasecmp(env_val, "true") == 0 || strcmp(env_val, "1") == 0);
    }
    ...
    if ((env_val = getenv("TQDM_COLOUR")) != NULL) {
        if (params->colour) free(params->colour);
...
tqdm_format_dict_t *tqdm_format_dict(tqdm_t *tqdm) {
    ...
    simple_dict.percentage = tqdm->params.total > 0 ? (100.0 * tqdm->n) / tqdm->params.total : 0.0;
...
char *tqdm_format_sizeof(double num, const char *suffix, int divisor) {
    ...
    if (num == (double)(long long)num && num < 1000000) {
        snprintf(result, 64, "%lld%s%s", 
                (long long)num, prefixes[prefix_idx], suffix_str);
    } else if (num >= 100 || prefix_idx == 0) {
        snprintf(result, 64, "%.0f%s%s", 
                num, prefixes[prefix_idx], suffix_str);
    } else if (num >= 10) {
        snprintf(result, 64, "%.1f%s%s", 
                num, prefixes[prefix_idx], suffix_str);
    } else {
        snprintf(result, 64, "%.2f%s%s", 
                num, prefixes[prefix_idx], suffix_str);
    }
...
char *tqdm_format_meter(size_t n, size_t total, double elapsed, 
    ...
    if (bar_format && strlen(bar_format) > 0) {
        ...
        snprintf(result, 256, "%s: %zu/%zu [%.1fs, %.1fit/s] %s", 
                prefix ? prefix : "", n, total, elapsed, rate, postfix ? postfix : "");
        return result;
    }
    ...
    if (prefix) estimated_fixed += strlen(prefix);
    if (postfix) estimated_fixed += strlen(postfix);
...
tqdm_t *tqdm_create_with_total(void *begin, size_t total, size_t element_size) {
    tqdm_params_t params = tqdm_default_params();
    params.total = total;
    tqdm_t *result = tqdm_create_with_params(begin, NULL, element_size, &params);
    tqdm_cleanup_params(&params);
    return result;
}

tqdm_t *tqdm_create_with_params(void *begin, void *end, size_t element_size, tqdm_params_t *user_params) {
    ...
    if (user_params->desc) {
        ...
        if (!tqdm->params.desc) goto cleanup_and_fail;
    } else {
    ...
    if (user_params->unit) {
        ...
        if (!tqdm->params.unit) goto cleanup_and_fail;
    } else {
    ...
    if (user_params->bar_format) {
        ...
        if (!tqdm->params.bar_format) goto cleanup_and_fail;
    } else {
    ...
    if (user_params->colour) {
        ...
        if (!tqdm->params.colour) goto cleanup_and_fail;
    } else {
    ...
    if (user_params->postfix) {
        ...
        if (!tqdm->params.postfix) goto cleanup_and_fail;
    } else {
        tqdm->params.postfix = NULL;
    }
    ...
    if (!tqdm->rate_history) goto cleanup_and_fail;
    ...
    if (tqdm->params.delay > 0) {
        ...
        delay_time.tv_nsec = (long)((tqdm->params.delay - delay_time.tv_sec) * 1e9);
...
void *tqdm_next(tqdm_t *tqdm) {
    if (!tqdm_has_next(tqdm)) return NULL;
    ...
    bool is_complete = (tqdm->params.total > 0 && tqdm->n >= tqdm->params.total);
    
    if (is_complete || 
        (tqdm->params.miniters == 0 || (tqdm->n - tqdm->last_print_count) >= tqdm->params.miniters)) {
        if (is_complete || current_time - tqdm->last_print_time >= tqdm->params.mininterval) {
            should_print = true;
        }
    }
...
void tqdm_update_n(tqdm_t *tqdm, size_t n) {
    if (!tqdm || tqdm->closed || tqdm->params.disable) return;
...
bool tqdm_update_to(tqdm_t *tqdm, size_t n) {
    if (!tqdm || tqdm->closed || tqdm->params.disable) return false;
...
void tqdm_refresh(tqdm_t *tqdm) {
    if (tqdm->closed || tqdm->params.disable) return;
...
tqdm_external_write_context_t *tqdm_external_write_mode(FILE *file, bool nolock) {
    tqdm_external_write_context_t *ctx = malloc(sizeof(tqdm_external_write_context_t));
    ctx->tqdm = NULL;
...
void tqdm_destroy(tqdm_t *tqdm) {
    ...
    if (tqdm->params.desc) free(tqdm->params.desc);
    if (tqdm->params.unit) free(tqdm->params.unit);
    if (tqdm->params.bar_format) free(tqdm->params.bar_format);
    if (tqdm->params.colour) free(tqdm->params.colour);
    if (tqdm->params.postfix) free(tqdm->params.postfix);
...
static void tqdm_print_progress(tqdm_t *tqdm) {
    if (!tqdm || tqdm->params.disable || tqdm->closed) return;
    ...
    char *meter = tqdm_format_meter(
        ...
        tqdm->params.ascii,
        tqdm->params.unit,
        tqdm->params.unit_scale,
        rate,
        tqdm->params.bar_format,
        tqdm->params.postfix,
        (int)tqdm->params.unit_divisor,
        tqdm->params.initial,
        tqdm->params.colour
    );
