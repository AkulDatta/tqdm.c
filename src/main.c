 ...
 * Printing helpers
 * ============================= */
static void print_version(void) { puts("tqdm " VERSION); }
...
static void print_help(void) {
  puts("Usage: tqdm [OPTIONS]\n"
       ...
       "  --leave / --no-leave      Leave progress bar after completion "
       "(default: leave)\n"
       ...
       "  --update-to               Treat each input line as an absolute "
       "value\n"
 ...
 * CLI parsing
 * ============================= */
static tqdm_params_t parse_args(int argc, char **argv,
                                processing_opts_t *popts) {
  *popts = proc_default();
  ...
  while ((opt = getopt_long(argc, argv,
                            "d:t:lLf:c:i:m:aDu:UNs:b:n:p:P:v:C:y:Be:z:TRSxhV",
                            long_opts, NULL)) != -1) {
    switch (opt) {
    /* Options */
    case 'd':
      free(p.desc);
      p.desc = strdup(optarg);
      break;
    case 't':
      p.total = (size_t)atoll(optarg);
      break;
    case 'l':
      p.leave = true;
      break;
    ...
    case 'c':
      ...
      break;
    case 'i':
      p.mininterval = atof(optarg);
      break;
    case 'm':
      p.miniters = (unsigned)atoi(optarg);
      break;
    case 'a':
      p.ascii = true;
      break;
    ...
    case 'N':
      ...
      break;
    case 's':
      p.smoothing = atof(optarg);
      break;
    case 'b':
      free(p.bar_format);
      p.bar_format = strdup(optarg);
      break;
    case 'n':
      p.initial = (size_t)atoll(optarg);
      break;
    case 'p':
      p.position = atoi(optarg);
      break;
    case 'P':
      free(p.postfix);
      p.postfix = strdup(optarg);
      break;
    case 'v':
      p.unit_divisor = atof(optarg);
      break;
    case 'C':
      free(p.colour);
      p.colour = strdup(optarg);
      break;
    case 'y':
      p.delay = atof(optarg);
      break;
    ...
    case 'e':
      if (!strcmp(optarg, "\\n"))
        popts->delim = '\n';
      else if (!strcmp(optarg, "\\0") || !strcmp(optarg, "0"))
        popts->delim = '\0';
      else
        popts->delim = optarg[0];
      break;
    case 'z':
      popts->buf_size = (size_t)atoll(optarg);
      break;
    case 'T':
      popts->tee = true;
      break;
...
static size_t process_updates(tqdm_t *bar, FILE *in, processing_opts_t *o) {
  ...
  while (fgets(line, sizeof(line), in)) {
    ...
    if (o->update_to)
      tqdm_update_to(bar, (size_t)val);
    else
      tqdm_update_n(bar, (size_t)val);
    ++processed;
    if (o->tee && o->null_ok == false)
      puts(line);
  }
  return processed;
}
...
int main(int argc, char **argv) {
  ...
  if (!bar) {
    fputs("Failed to create tqdm instance\n", stderr);
    return 1;
  }

  FILE *input = stdin;
  if (isatty(STDIN_FILENO))
    fputs("Reading from terminal (Ctrl+D to end)\n", stderr);

  size_t count = 0;
  if (proc_opts.update || proc_opts.update_to)
    count = process_updates(bar, input, &proc_opts);
  else
    count = process_stream(bar, input, &proc_opts);
