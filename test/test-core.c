...
#define TEST_ASSERT(condition, message)                                      \
  do {                                                                       \
    if (!(condition)) {                                                      \
      snprintf(last_error, sizeof(last_error), "Assertion failed: %s",       \
               message);                                                     \
      goto test_fail;                                                        \
    }                                                                        \
  } while (0)
...
#define TEST_ASSERT_NE(a, b, message) TEST_ASSERT((a) != (b), message)
...
void test_iteration(void) {
  ...
  TEST_ASSERT_EQ(tqdm->n, TEST_SIZE,
                 "Progress counter should match iteration count");
...
void test_range(void) {
  ...
  TEST_ASSERT_EQ(
      count, 7,
      "Should iterate through correct number of steps");  // 0,3,6,9,12,15,18
...
void test_params(void) {
  ...
  TEST_ASSERT_NOT_NULL(tqdm,
                       "tqdm_create_with_params should not return NULL");

  // Verify parameters were set
  TEST_ASSERT_STR_EQ(tqdm->params.desc, "Custom test",
  ...
  TEST_ASSERT_FLOAT_EQ(tqdm->params.mininterval, 0.05f, 0.001f,
                       "Min interval should be set");
...
void test_postfix(void) {
  ...
  TEST_ASSERT_NULL(postfix,
                   "postfix_create should return NULL for empty list");
...
void test_edge(void) {
  ...
  TEST_ASSERT_EQ(tqdm->n, 0,
                 "Disabled progress bar should not track updates");
...
void test_threading(void) {
  ...
  // Test basic thread safety features (without actual threading for
  // simplicity)
  tqdm_params_t params = tqdm_default_params();
...
void test_format(void) {
  ...
  TEST_ASSERT(strstr(size_str, "1.5") != NULL,
              "Should contain '1.5' for 1536 bytes");
  free(size_str);

  // Test interval formatting
  char *interval_str = tqdm_format_interval(3661.5);
  TEST_ASSERT_NOT_NULL(interval_str,
                       "format_interval should not return NULL");
  ...
  char *meter =
      tqdm_format_meter(750, 1000, 30.5, 80, "Processing", false, "items",
                        false, 24.6, NULL, "acc=0.95", 1000, 0, NULL);
  TEST_ASSERT_NOT_NULL(meter, "format_meter should not return NULL");
  TEST_ASSERT(strstr(meter, "Processing") != NULL,
...
void test_env(void) {
  ...
  TEST_ASSERT_FLOAT_EQ(params.mininterval, 0.2f, 0.001f,
                       "Should load mininterval from env");
  ...
  // Reset env
  if (orig_mininterval)
    setenv("TQDM_MININTERVAL", orig_mininterval, 1);
  if (orig_unit)
    setenv("TQDM_UNIT", orig_unit, 1);
